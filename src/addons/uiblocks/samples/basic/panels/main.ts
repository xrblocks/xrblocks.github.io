...
import {Paint, UIPanel, UIPanelProperties, UIText} from 'uiblocks';
...
class PanelSample extends Sample {
   ...
   */
  private createPanelWithLabel(
    parent: UIPanel,
    label: string,
    props: UIPanelProperties
  ) {
  ...
  createUI() {
    ...
    const card1 = this.uiCore.createCard({
      ...
      position: new THREE.Vector3(0, 1.6, -1.0),
      sizeX: 1.5,
      sizeY: 1.0,
      flexDirection: 'column',
      justifyContent: 'center',
    ...
    const row1 = this.createSectionWithTitle(
      card1,
      'Panel Colors (Solid & Gradient)'
    );
    ...
    // Row 2: Inner Shadows.
    const row2 = this.createSectionWithTitle(
      card1,
      'Inner Shadows (Blur, Spread, Falloff, Position, Gradient)'
    );
    ...
    // Row 3: Drop Shadows.
    const row3 = this.createSectionWithTitle(
      card1,
      'Drop Shadows (Blur, Spread, Falloff, Position, Gradient)'
    );
    ...
    const row4 = this.createSectionWithTitle(
      card1,
      'Strokes (Solid, Width, Align, Gradient)'
    );
    ...
    const dynamicRow = this.createSectionWithTitle(
      card2,
      'Dynamic Panel Properties'
    );

    // 1. Dynamic Gradient.
    this.dynamicPanel1 = this.createPanelWithLabel(
      dynamicRow,
      'Linear Gradient Fill Rot',
      {
        fillColor: {
          gradientType: 'linear',
          rotation: 0,
    ...
    // 2. Dynamic Radius.
    this.dynamicPanel2 = this.createPanelWithLabel(
      dynamicRow,
      'Corner Radius',
      {
        cornerRadius: 0,
    ...
    this.dynamicPanel3 = this.createPanelWithLabel(
      dynamicRow,
      'Inner Shadow Blur',
      {
        fillColor: '#1a1a1a',
    ...
    this.dynamicPanel4 = this.createPanelWithLabel(
      dynamicRow,
    ...
    this.dynamicPanel5 = this.createPanelWithLabel(
      dynamicRow,
      'Angular Gradient Stroke Rot',
      {
        cornerRadius: 50,
        fillColor: '#1a1a1a',
        strokeWidth: 6,
  ...
  // Define properties for dynamic panels.
  private dynamicPanel1!: UIPanel;
  private dynamicPanel2!: UIPanel;
  private dynamicPanel3!: UIPanel;
  private dynamicPanel4!: UIPanel;
  private dynamicPanel5!: UIPanel;
}
