...
import {UIPanel, UIPanelProperties, UIText} from 'uiblocks';
...
class LayoutSample extends Sample {
  ...
  createUI() {
    ...
    // 1. Flex Direction.
    const dirSection = this.createSectionWithTitle(
      card,
      'Flex Direction',
      '100%',
      'auto',
    ...
    // 2. Justify Content.
    const justifySection = this.createSectionWithTitle(
      card,
      'Justify Content',
      '100%',
      'auto',
    ...
    // Flex Start.
    const startContainer = this.createFlexContainer(
      justifySection,
      'Flex Start',
      {
        justifyContent: 'flex-start',
    ...
    // Space Between.
    const spaceBetweenContainer = this.createFlexContainer(
      justifySection,
      'Space Between',
      {
    ...
    const alignSection = this.createSectionWithTitle(
      card,
      'Align Items (in 80px height)',
      '100%',
      'auto',
      'column'
    );
    ...
    const advancedSection = this.createSectionWithTitle(
      card,
      'Advanced Flex',
      '100%',
      'auto',
    ...
    // Flex Grow.
    const growContainer = this.createFlexContainer(
      advancedSection,
      'Flex Grow',
      {
        gap: 10,
    ...
    // Flex Basis.
    const basisContainer = this.createFlexContainer(
      advancedSection,
      'Flex Basis',
      {
        gap: 10,
