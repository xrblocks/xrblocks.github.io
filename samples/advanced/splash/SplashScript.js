...
export class SplashScript extends xb.Script {
  ...
  controllerUpdate(controller) {
    ...
    if (now - this.lastBallTime.get(controller) >= 1000 / kBallsPerSecond) {
      ...
      this.velocity
        ...
        .applyQuaternion(this.controllerQuaternion);
      this.ballShooter.spawnBallAt(this.ballPosition, this.velocity);
      this.lastBallTime.set(controller, now);
    }
  }

  onSelecting(event) {
    this.controllerUpdate(event.source.controller);
  }

  update() {
    for (const paintball of this.paintballs) {
      paintball.update();
    }
  }
