...
export class PortalGalleryScene extends xb.Script {
  ...
  async init() {
    ...
    this._exitLabel = new xb.UICard({
      size: {width: 0.7, height: 0.22},
      pointerEvents: 'none',
    });
    this._exitLabel.name = 'Exit label';
    this._exitLabel.add(
      new xb.UIText({
        text: '↩ EXIT',
        pointerEvents: 'none',
  ...
  onSelectStart(event) {
    ...
    const selectedPortal = this.portals.find(
      (portal) => event.surface === portal._disc
    );
    ...
    if (this._held) {
      ...
      if (depthMesh && event.surface === depthMesh && event.intersection) {
        this._held.placeAt(
          event.intersection.point,
          event.intersection.normal,
          event.surface.matrixWorld
        );
        this._held.setHeld(false);
        this._held = null;
      }
    ...
    if (selectedPortal) {
      this._held = selectedPortal;
      selectedPortal.setHeld(true);
    }
