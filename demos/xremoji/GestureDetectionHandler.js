...
export class GestureDetectionHandler {
    ...
    calculateRelativeHandBoneAngles(jointPositions) {
        ...
        HAND_JOINT_IDX_CONNECTION_MAP.forEach(([joint1, joint2]) => {
            const boneVector = jointPositionsReshaped.slice([joint2, 0], [1, 3]).sub(
                jointPositionsReshaped.slice([joint1, 0], [1, 3])
            ).squeeze();
    ...
    async detectGesture(handJoints) {
        ...
        try {
            ...
            let tensorReshaped = tensor.reshape([1, HAND_BONE_IDX_CONNECTION_MAP.length, 1]);
            var result = -1;
