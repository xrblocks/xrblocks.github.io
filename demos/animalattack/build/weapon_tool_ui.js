...
import { createSpatialPanel, buildSpatialActionPanel } from './spatial_action_panel.js';
...
class WeaponToolUI {
    ...
    constructor(scene, onToggleCallback, onRainToggleCallback) {
        ...
        this.sourcePanel = createSpatialPanel(PANEL_WIDTH, PANEL_HEIGHT, new THREE.Vector3(PANEL_POS_X, PANEL_POS_Y, PANEL_POS_Z), PANEL_ROT_X);
        ...
        const ring = new THREE.Mesh(new THREE.TorusGeometry(RING_RADIUS, RING_TUBE, RING_RADIAL_SEGMENTS, RING_TUBULAR_SEGMENTS), sourceMat);
        const vert = new THREE.Mesh(new THREE.BoxGeometry(VERT_WIDTH, VERT_HEIGHT, VERT_DEPTH), sourceMat);
        const horz = new THREE.Mesh(new THREE.BoxGeometry(HORZ_WIDTH, HORZ_HEIGHT, HORZ_DEPTH), sourceMat);
        ...
        const weaponUi = buildSpatialActionPanel(scene, new THREE.Vector3(WEAPON_PANEL_POS_X, PANEL_POS_Y, PANEL_POS_Z), 'casino', this.bgMeshes, () => this.toggleWeapons());
        this.panels.push(weaponUi.panel);
        this.weaponBtn = weaponUi.bgMesh;
        const rainUi = buildSpatialActionPanel(scene, new THREE.Vector3(RAIN_PANEL_POS_X, PANEL_POS_Y, PANEL_POS_Z), 'pets', this.bgMeshes, () => {
    ...
    updatePanelState(bgMesh, isActive, activeColorHex) {
        bgMesh.material.color.setHex(isActive ? activeColorHex : INACTIVE_COLOR);
        bgMesh.userData.edgeLine.material.color.setHex(isActive ? ACTIVE_EDGE_COLOR : INACTIVE_EDGE_COLOR);
    }
    ...
    toggleWeapons(forceEnable = false) {
        ...
        this.sourceIcon.userData.mat.color.setHex(this.weaponsEnabled ? WEAPON_ACTIVE_COLOR : WEAPON_INACTIVE_COLOR);
