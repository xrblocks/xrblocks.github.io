...
const LASER_CYLINDER_GEO = new THREE.CylinderGeometry(1, 1, 1, LASER_CYLINDER_SEGMENTS).rotateX(Math.PI / 2);
const BALL_GLOW_GEO = new THREE.SphereGeometry(1, BALL_GLOW_SEGMENTS, BALL_GLOW_SEGMENTS);
const BALL_CORE_GEO = new THREE.SphereGeometry(1, BALL_CORE_SEGMENTS, BALL_CORE_SEGMENTS);
...
const BALL_COLOR_POOL = Array.from({ length: 50 }, () => new THREE.Color().setHSL(Math.random(), 1.0, 0.5));
...
class WeaponEffects {
    ...
    static show(start, end, scene, weaponType) {
        AudioManager.playZapSound();
        const distance = start.distanceTo(end);
        if (distance < MIN_FIRE_DISTANCE)
            return;
        ...
        switch (weaponType) {
            case 'laser':
                ...
                break;
            case 'ball':
                this.createBall(start, end, direction, weaponGroup, meshesToFade, animatedMeshes);
                break;
            case 'fire':
                this.createFire(start, end, direction, weaponGroup, meshesToFade, animatedMeshes);
                break;
            case 'tornado':
                this.createTornado(start, end, direction, weaponGroup, meshesToFade, animatedMeshes);
                break;
        }
        this.animateEffect(scene, weaponGroup, meshesToFade, animatedMeshes, weaponType);
    }
    /** Builds a laser beam composed of a core and glowing outer layers. */
    static createLaser(start, end, distance, weaponGroup, meshesToFade) {
        const midPoint = start.clone().lerp(end, 0.5);
        const laserLineGroup = new THREE.Group();
        laserLineGroup.position.copy(midPoint);
        laserLineGroup.lookAt(end);
        const createCylinder = (radius, baseMat, color, opacity) => {
        ...
        const core = createCylinder(LASER_CORE_RADIUS, BASE_LASER_CORE_MAT, LASER_CORE_COLOR, LASER_CORE_OPACITY);
        const innerGlow = createCylinder(LASER_INNER_RADIUS, BASE_LASER_GLOW_MAT, LASER_INNER_COLOR, LASER_INNER_OPACITY);
        const outerGlow = createCylinder(LASER_OUTER_RADIUS, BASE_LASER_GLOW_MAT, LASER_OUTER_COLOR, LASER_OUTER_OPACITY);
    ...
    static createBall(start, end, direction, weaponGroup, meshesToFade, animatedMeshes) {
        const glowMat = BASE_BALL_GLOW_MAT.clone();
        const coreMat = BASE_BALL_CORE_MAT.clone();
        const glowInstanced = new THREE.InstancedMesh(BALL_GLOW_GEO, glowMat, BALL_PARTICLE_COUNT);
        const coreInstanced = new THREE.InstancedMesh(BALL_CORE_GEO, coreMat, BALL_PARTICLE_COUNT);
        ...
        meshesToFade.push({ mat: glowMat, maxOpacity: BALL_GLOW_OPACITY }, { mat: coreMat, maxOpacity: BALL_CORE_OPACITY });
        for (let i = 0; i < BALL_PARTICLE_COUNT; i++) {
            const progress = i / (BALL_PARTICLE_COUNT - 1) ;
    ...
    static createFire(start, end, direction, weaponGroup, meshesToFade, animatedMeshes) {
        const fireExtend = Math.max(start.distanceTo(end) * FIRE_EXTEND_MULTIPLIER, FIRE_EXTEND_MIN);
        const extendedEnd = sharedVector
            .copy(start)
            .addScaledVector(direction, fireExtend);
        const fireMat = BASE_FIRE_MAT.clone();
        meshesToFade.push({ mat: fireMat, maxOpacity: FIRE_OPACITY });
        const fireInstanced = new THREE.InstancedMesh(FIRE_PARTICLE_GEO, fireMat, FIRE_PARTICLE_COUNT);
        ...
        for (let i = 0; i < FIRE_PARTICLE_COUNT; i++) {
            ...
            fireInstanced.setColorAt(i, FIRE_COLOR_OBJS[randInt(FIRE_COLOR_OBJS.length)]);
            animatedMeshes.push({
                type: 'fire',
                mesh: fireInstanced,
                index: i,
                ...
                speed: FIRE_SPEED,
                dir: direction,
                rotSpeed: new THREE.Vector3(THREE.MathUtils.randFloatSpread(FIRE_ROT_SPREAD), THREE.MathUtils.randFloatSpread(FIRE_ROT_SPREAD), THREE.MathUtils.randFloatSpread(FIRE_ROT_SPREAD)),
            });
        }
    ...
    static createTornado(start, end, direction, weaponGroup, meshesToFade, animatedMeshes) {
        const fireExtend = Math.max(start.distanceTo(end) * FIRE_EXTEND_MULTIPLIER, FIRE_EXTEND_MIN);
        const extendedEnd = sharedVector
            .copy(start)
            .addScaledVector(direction, fireExtend);
        ...
        const upDir = new THREE.Vector3()
            ...
            .normalize();
        const tornadoMat = BASE_TORNADO_MAT.clone();
        meshesToFade.push({ mat: tornadoMat, maxOpacity: TORNADO_OPACITY });
        const tornadoInstanced = new THREE.InstancedMesh(TORNADO_PARTICLE_GEO, tornadoMat, TORNADO_PARTICLE_COUNT);
        ...
        for (let i = 0; i < TORNADO_PARTICLE_COUNT; i++) {
            ...
            const size = THREE.MathUtils.randFloat(TORNADO_MIN_SIZE, TORNADO_MAX_SIZE);
            const radius = Math.min(progress * TORNADO_RADIUS_MULTIPLIER, TORNADO_RADIUS_MAX) +
                TORNADO_RADIUS_BASE;
            ...
            tornadoInstanced.setColorAt(i, TORNADO_COLOR_OBJS[randInt(TORNADO_COLOR_OBJS.length)]);
            animatedMeshes.push({
                type: 'tornado',
                ...
                upDir,
                jitter: radius * 0.1,
                rotSpeed: new THREE.Vector3(THREE.MathUtils.randFloatSpread(TORNADO_ROT_SPREAD), THREE.MathUtils.randFloatSpread(TORNADO_ROT_SPREAD), THREE.MathUtils.randFloatSpread(TORNADO_ROT_SPREAD)),
                angle,
                radius,
                spinSpeed,
                size,
    ...
    static animateEffect(scene, weaponGroup, meshesToFade, animatedMeshes, weaponType) {
        let opacity = 1.0;
        const fadeSpeed = weaponType === 'laser'
            ? LASER_FADE_SPEED
            : weaponType === 'tornado'
                ? TORNADO_FADE_SPEED
                : DEFAULT_FADE_SPEED;
    ...
    static cleanupGroup(scene, group) {
        scene.remove(group);
    ...
    static updateFades(meshesToFade, opacity) {
    ...
    static updateAnimations(animatedMeshes, fps) {
        updatedMeshes.clear();
    ...
    static animateTornadoItem(item, fps) {
        const { centerPos, right, upDir, rotSpeed, dir, speed, radius, spinSpeed, jitter, size, } = item;
        centerPos.addScaledVector(dir, speed * (fps / MS_PER_SEC));
        item.angle += spinSpeed * (fps / MS_PER_SEC);
        const offsetX = Math.cos(item.angle) * radius + THREE.MathUtils.randFloatSpread(jitter);
        const offsetY = Math.sin(item.angle) * radius + THREE.MathUtils.randFloatSpread(jitter);
        sharedVector
            .copy(right)
            .multiplyScalar(offsetX)
            .addScaledVector(upDir, offsetY);
    ...
    static animateFireItem(item, fps) {
    ...
    static animateBallItem(item, fps) {
    ...
    static animateBaseItem(item, fps) {
        const { dir, speed } = item;
        item.pos.addScaledVector(dir, speed * (fps / MS_PER_SEC));
        sharedObject.position.copy(item.pos);
        sharedObject.rotation.set(0, 0, 0);
