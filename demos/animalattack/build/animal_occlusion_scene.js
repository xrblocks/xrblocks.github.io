...
class AnimalOcclusionScene extends xb.Script {
    ...
    addLights() {
        this.add(new THREE.HemisphereLight(LIGHT_HEMISPHERE_SKY, LIGHT_HEMISPHERE_GROUND, LIGHT_HEMISPHERE_INTENSITY));
        const light = new THREE.DirectionalLight(LIGHT_DIR_COLOR, LIGHT_DIR_INTENSITY);
    ...
    startDraggingAnimal(index, startPosition) {
        ...
        if (this.draggedAnimal) {
            ...
            if (startPosition)
                this.draggedAnimal.position.copy(startPosition);
        }
    }
    ...
    dropAnimal(raycaster) {
        if (!this.draggedAnimal)
            return;
        const paletteHit = InteractionUtils.checkIntersection(raycaster, this.sliderUI.getPaletteItems(), this.spawnedAnimals);
        if (paletteHit?.object.userData?.isPaletteItem) {
    ...
    explodeAllAnimals() {
        for (const [index, animal] of this.spawnedAnimals.entries()) {
            if (animal.position.y > ACTIVE_Y_THRESHOLD)
    ...
    handleInteractionStart(raycaster, sourceObject, event) {
        ...
        if (this.isDragging)
            return;
        const hit = InteractionUtils.checkIntersection(raycaster, this.sliderUI.getPaletteItems(), this.spawnedAnimals);
        if (!hit)
            return;
    ...
    getAvailableAnimalFromPalette(typeIndex) {
        const pool = [...this.spawnedAnimals.entries()].filter(([, a]) => a.userData.typeIndex === typeIndex);
        for (const [, a] of pool) {
