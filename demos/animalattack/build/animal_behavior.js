...
class AnimalBehavior {
    ...
    static updateDragTransform(isDragging, draggedAnimal, dragSource, raycaster, pointer, camera, depthMesh, dragDistance) {
        ...
        const isEnvHit = envHits.length > 0 && envHits[0].distance < ENV_HIT_DISTANCE_MAX;
        ...
        if (isEnvHit)
            lookTarget.y = draggedAnimal.position.y;
    ...
    static updateWandering(spawnedAnimals, depthMesh, deltaTime, time, draggedAnimal, camera) {
        ...
        for (const animal of spawnedAnimals.values()) {
            ...
            if (userData.hasTarget) {
                const dist = Math.hypot(userData.targetPosition.x - position.x, userData.targetPosition.y - position.y, userData.targetPosition.z - position.z);
                if (dist < TARGET_REACHED_DISTANCE)
                    userData.hasTarget = false;
            }
            if (!userData.hasTarget) {
                this.calculateWanderTarget(animal, camera, depthMesh, sharedRaycaster, downVector);
            }
            if (userData.hasTarget) {
    ...
    static computeNextPosition(animal, isDash, verticalAction) {
        ...
        const vertRange = isDash
            ? THREE.MathUtils.randFloat(WANDER_VERT_RANGE_DASH_MIN, WANDER_VERT_RANGE_DASH_MAX)
            : THREE.MathUtils.randFloat(WANDER_VERT_RANGE_NORMAL_MIN, WANDER_VERT_RANGE_NORMAL_MAX);
    ...
    static applyWanderConstraints(animal, pos, spreadArea, depthMesh, raycaster, downVector) {
        const { homeAnchor } = animal.userData;
        const newX = THREE.MathUtils.clamp(pos.x, homeAnchor.x - spreadArea, homeAnchor.x + spreadArea);
        const newZ = THREE.MathUtils.clamp(pos.z, homeAnchor.z - spreadArea, homeAnchor.z + spreadArea);
        let newY = THREE.MathUtils.clamp(pos.y, homeAnchor.y - WANDER_Y_SPREAD_MIN, homeAnchor.y + WANDER_Y_SPREAD_MAX);
        if (depthMesh) {
            raycaster.set(sharedVectorA.set(newX, animal.position.y + 2.0, newZ), downVector);
    ...
    static setupParabolicMovement(animal, isDash) {
        ...
        if (Math.abs(moveDir.normalize().y) > 0.8)
            arcAxis.set(1, 0, 0);
        ...
        const arcHeight = (Math.random() * ARC_HEIGHT_MULT + ARC_HEIGHT_MIN) * moveDist;
    ...
    static setupTargetRotation(animal) {
    ...
    static calculateWanderTarget(animal, _camera, depthMesh, raycaster, downVector) {
        const { userData, position } = animal;
        ...
        userData.turnSpeed = THREE.MathUtils.randFloat(TURN_SPEED_MIN, TURN_SPEED_MAX);
        const targetPos = this.computeNextPosition(animal, isDash, verticalAction);
        const constrainedPos = this.applyWanderConstraints(animal, targetPos, spreadArea, depthMesh, raycaster, downVector);
    ...
    static moveTowardsTarget(animal, deltaTime, time) {
        const { userData, position, quaternion } = animal;
        if (userData.isTurning) {
            quaternion.slerp(userData.targetQuat, deltaTime * userData.turnSpeed);
            if (1.0 - Math.abs(quaternion.dot(userData.targetQuat)) <
    ...
    static updateFalling(spawnedAnimals, depthMesh, particleSystem, camera, deltaTime, draggedAnimal) {
        for (const animal of spawnedAnimals.values()) {
            const { position, userData } = animal;
            if (position.y > ACTIVE_Y_THRESHOLD &&
                userData.isFalling &&
                ...
                let groundY = camera
                    ...
                    : -FALL_GROUND_OFFSET;
                if (depthMesh) {
                    sharedRaycaster.set(sharedVectorA.set(position.x, position.y + 1, position.z), sharedVectorB.set(0, -1, 0));
