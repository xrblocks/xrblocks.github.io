...
import { BroadcastChannelTransport, AVATAR_PALETTE, hashStringToIndex } from 'xrblocks/addons/netblocks/src/index.js';
...
class IntegrationSample extends NetSample {
    _displayName = `User-${Math.floor(Math.random() * 1000)}`;
    _cubes = [];
    _drag = null;
    _voiceOn = false;
    _log;
    ...
    _buildSpatialHud(session) {
        ...
        const keyboard = new Keyboard({
            onValueChange: (text) => {
                if (this._spatialDraft)
                    this._spatialDraft.text = `› ${text}`;
            },
            onSubmit: (text) => {
