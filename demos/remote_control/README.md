...
node demos/remote_control/send.mjs get-camera
...
```bash
node demos/remote_control/send.mjs tool getSimulatorState
node demos/remote_control/send.mjs tool getCamera '{"screenshot":true}'
node demos/remote_control/send.mjs tool nudgeCube '{"dx":0.25}'
```
...
```text
http://127.0.0.1:8080/demos/remote_control/?remoteControlUrl=ws://127.0.0.1:9000
```

```bash
REMOTE_CONTROL_URL=ws://127.0.0.1:9000 node demos/remote_control/send.mjs get-state
```

## Troubleshooting

If the page reports that `RemoteControl.configureOptions` is missing, rebuild
the addon bundle. The demo now uses the generic
...
The demo imports from `build/addons/remote-control/`, not directly from
`src/`.
